...
```{r}
library(dplyr)
select(cran, c(ip_id, package, country))
```
...
cran %>% filter(country %in% c("ES", "PT"))
...
```{r}
cran2 %>% arrange(desc(r_version)) %>% arrange(country,ip_id)
...
```{r}
cran%>% summarise(avg_bytes= mean(size, na.rm = TRUE))
```
# Agrupamientos
